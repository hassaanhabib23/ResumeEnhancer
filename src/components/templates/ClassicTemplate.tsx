...
// list in order. Skills/Languages are laid out side-by-side in a 2-col
// grid when they land next to each other, but that's a visual pairing,
// not a separate zone (see the `grid2col` handling below).
...
export default function ClassicTemplate({
  ...
  const skillsNode = (editable || data.skills.length > 0) && (
    <section>
  ...
  // Skills and Languages render as a paired 2-col grid when they land next
  // to each other in the resolved order (matching the template's original
  // look); otherwise each renders as its own full-width section in place.
  const rendered: React.ReactNode[] = []
  for (let i = 0; i < order.length; i++) {
    const key = order[i]
    const next = order[i + 1]
    if (key === 'skills' && next === 'languages') {
      rendered.push(
        <div className="mt-7 grid grid-cols-2 gap-8" key="skills-languages-pair">
          {skillsNode}
          {languagesNode}
        </div>,
      )
    ...
    if (key === 'languages' && next === 'skills') {
      rendered.push(
        <div className="mt-7 grid grid-cols-2 gap-8" key="languages-skills-pair">
          {languagesNode}
          {skillsNode}
        </div>,
      )
    ...
    if (key === 'skills') rendered.push(<div className="mt-7" key="skills">{skillsNode}</div>)
    else if (key === 'languages')
      rendered.push(<div className="mt-7" key="languages">{languagesNode}</div>)
  ...
  return (
    ...
    >
      <header className="text-center">
        <h1 className="text-[26px] font-semibold tracking-tight" style={{ fontFamily: fonts.heading }}>
          <EditableText
            editable={editable}
        ...
        <p className="mt-1 text-[13px]" style={{ color: theme.accent }}>
          <EditableText
            editable={editable}
            value={contact.title}
        ...
        <div
          className="mx-auto mt-3 flex max-w-xl flex-wrap items-center justify-center gap-x-3 gap-y-1 text-[10.5px] text-ink-500"
        ...
        <div className="mx-auto mt-6 h-px w-full" style={{ background: theme.accent, opacity: 0.3 }} />
      </header>

      <div style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>{rendered}</div>
    </div>
  )
}
