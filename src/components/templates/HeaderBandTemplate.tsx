...
export default function HeaderBandTemplate({
  ...
  const sidebarNodes: Partial<Record<ReorderableSection, React.ReactNode>> = {
    ...
    languages: (editable || data.languages.length > 0) && (
      <div className="mb-7" key="languages">
        <SectionTitle color={theme.accent} font={fonts.heading}>
          Languages
        </SectionTitle>
  ...
  return (
    ...
    >
      <div className="relative">
        <div
          style={{ background: theme.accent, paddingRight: hasPhoto ? 176 : 40 }}
          className="flex h-40 flex-col justify-center px-10"
        >
          <h1
          ...
          <p className="mt-2 text-[15px] font-medium" style={{ color: 'rgba(255,255,255,0.85)' }}>
            <EditableText
              editable={editable}
              value={contact.title}
        ...
        {hasPhoto && (
          <div className="absolute right-10 z-10" style={{ top: 96 }}>
            <img
              src={contact.photo}
              alt=""
      ...
      <div style={{ paddingTop: hasPhoto ? 78 : 22 }} className="px-10 pb-3">
        {(editable ||
          contact.email ||
          contact.phone ||
          contact.location ||
          ...
          contact.linkedin ||
          contact.github) && (
          <div className="flex flex-wrap gap-x-5 gap-y-1.5 text-[11px] text-ink-500">
      ...
      <div className="flex px-2 pb-10">
        <aside className="w-[38%] px-8 pt-4">{sidebarOrder.map((key) => sidebarNodes[key])}</aside>

        <main className="w-[62%] border-l border-ink-100 px-8 pt-4">
