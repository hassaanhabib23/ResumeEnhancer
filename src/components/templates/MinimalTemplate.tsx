...
function SectionTitle({
  ...
  return (
    <h2
      className="mb-3 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.15em] text-ink-800"
...
export default function MinimalTemplate({
  ...
  const nodes: Partial<Record<ReorderableSection, React.ReactNode>> = {
    summary: (editable || data.summary) && (
      <section key="summary">
        <SectionTitle color={theme.accent} font={fonts.heading}>About</SectionTitle>
      ...
      <section key="skills">
        ...
        <div className="flex flex-wrap gap-2">
          {data.skills.map((s) => (
            <span
              key={s.id}
              className="rounded-full border px-3 py-1 text-[10.5px]"
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    if (key === 'certifications' && next === 'languages') {
      if (certLangPairVisible) {
        rendered.push(
          <div className="grid grid-cols-2 gap-6" key="certifications-languages-pair">
            {certNode}
            {langNode}
          </div>,
        )
    ...
    if (key === 'languages' && next === 'certifications') {
      if (certLangPairVisible) {
        rendered.push(
          <div className="grid grid-cols-2 gap-6" key="languages-certifications-pair">
            {langNode}
            {certNode}
          </div>,
        )
  ...
  return (
    ...
    >
      <header className="border-b border-ink-100 pb-6">
        <h1 className="text-[24px] font-bold tracking-tight" style={{ fontFamily: fonts.heading }}>
          <EditableText
            editable={editable}
        ...
        <div className="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-[10.5px] text-ink-500">
